...
class FileIconService {
  ...
  iconClassForPath(path) {
    const result = this._iconClassForPath(path);
    console.log("***", path, "=>", result);
    return result;
  }

  _iconClassForPath(path) {
    const [dir, filePath] = atom.project.relativizePath(path);
    const project = this._getProject(dir);
    classes = "icon-code-reader";
    if (project.hasColor(filePath, "red")) {
      classes += " red";
    } else if (project.hasColor(filePath, "blue")) {
      classes += " blue";
    } else if (project.hasColor(filePath, "green")) {
      classes += " green";
    }
    if (project.isMostlyColored(filePath)) {
      classes += " mostly-colored";
    }
    return classes;
  }
  ...
  _getProject(dir) {
    if (!this.projects[dir]) {
      this.projects[dir] = CodeReaderProject.load(dir);
      setTimeout(() => delete this.projects[dir], 10000);
    }
    return this.projects[dir];
  }
}
...
class CodeReader {
  ...
  provideFileIconsService() {
    return new FileIconService();
  }

  activate(state) {
    this.colors = CodeReaderProject.colors();
  ...
  _colorize_selection(color) {
    ...
    this.project.setLineCount(path, lineCount);
